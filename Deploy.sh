...
retrieve_default_vpc_and_subnets() {
  ...
  # Get Default VPC ID using JMESPath join to create a comma-separated list
  ...
  # Get Default Subnets IDs using JMESPath join to create a comma-separated list
...
create_security_group() {
  ...
  # Allow inbound HTTP traffic on port 80 if not already allowed
  ...
  if [ -z "$INGRESS_RULE_EXISTS" ]; then
    aws ec2 authorize-security-group-ingress \
      ...
      --cidr 0.0.0.0/0 \
      --region "$AWS_REGION"
    echo "Inbound rule added to allow HTTP traffic on port $CONTAINER_PORT."
  else
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    --desired-count) DESIRED_COUNT="$2"; shift ;;
